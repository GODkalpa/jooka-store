...
var AddReturnItemsTable = ({
  ...
  const alreadyAdded = useMemo2(() => new Set(selectedItems2), [selectedItems2]);
  const updater = (fn) => {
  ...
  const { table } = useDataTable({
    ...
    enableRowSelection: (row) => {
      return getReturnableQuantity(row.original) > 0 && !alreadyAdded.has(row.original.id);
    },
    rowSelection: {
      state: rowSelection,
      updater
...
var ReturnCreateForm = ({
  ...
  const onShippingOptionChange = async (selectedOptionId) => {
    const promises = preview.shipping_methods.map((s) => s.actions?.find((a) => a.action === "SHIPPING_ADD")?.id).filter(Boolean).map((id) => deleteReturnShipping(id));
  ...
  const estimatedDifference = preview.summary.pending_difference - previewItems.reduce((acc, item) => {
...
var ReturnCreate = () => {
  ...
  useEffect2(() => {
    async function run() {
      ...
      try {
        const { return: orderReturn } = await initiateReturn({
          order_id: order.id
        });
