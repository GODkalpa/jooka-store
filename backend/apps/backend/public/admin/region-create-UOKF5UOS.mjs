...
var RegionCreate = () => {
  ...
  const storeCurrencies = (store?.supported_currencies ?? []).map((c) => currencies[c.currency_code.toUpperCase()]).filter(Boolean);
  if (isError) {
    throw error;
  }
  return /* @__PURE__ */ jsx2(RouteFocusModal, { children: !isLoading && store && /* @__PURE__ */ jsx2(CreateRegionForm, { currencies: storeCurrencies }) });
};
export {
